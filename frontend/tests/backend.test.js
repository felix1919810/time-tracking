...
function backend() {
  ...
  const context={require:name=>name==='express'?express:name==='cors'?()=>()=>{}:name==='axios'?axios:name==='./passwords.cjs'?require('../api-deploy/passwords.cjs'):name==='./input-security.cjs'?require('../api-deploy/input-security.cjs'):name==='./import.cjs'?require('../api-deploy/import.cjs'):name==='./history.cjs'?require('../api-deploy/history.cjs'):name==='./auth.cjs'?require('../api-deploy/auth.cjs'):name==='./translation.cjs'?require('../api-deploy/translation.cjs'):['path','crypto','https','zlib'].includes(name)?require(name):name==='iconv-lite'?{decode:b=>b.toString()}:(()=>{throw Error('Unexpected dependency '+name)})(),__dirname:'mock-public',process:{env:{}},Buffer,URLSearchParams,console:{log(){},error(){}},module:{exports:{}},Date}
