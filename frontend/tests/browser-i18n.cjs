...
;(async () => {
  ...
  try {
    ...
    await page.getByRole('radio',{name:'Automatic day/night'}).check()
    await page.reload();await page.locator('.settings-view').waitFor()
    assert(await page.getByRole('radio',{name:'Automatic day/night'}).isChecked())
    const automatic=await page.evaluate(()=>({saved:localStorage.getItem('tt_theme'),actual:document.documentElement.dataset.theme,expected:new Date().getHours()>=7&&new Date().getHours()<19?'light':'dark'}))
    assert.equal(automatic.saved,'auto');assert.equal(automatic.actual,automatic.expected)
    await page.screenshot({path:path.join(__dirname,'../../artifacts/settings-auto.png'),fullPage:true})
    await page.getByRole('radio',{name:'Light mode'}).check()
    ...
    await page.screenshot({path:path.join(__dirname,'../../artifacts/mobile-reports.png'),fullPage:true})
    const mobileWidth=await page.evaluate(()=>({viewport:document.documentElement.clientWidth,content:document.documentElement.scrollWidth}))
    assert(mobileWidth.content <= mobileWidth.viewport + 1,JSON.stringify(mobileWidth))
    assert(await page.locator('.sidebar').evaluate(el=>el.getBoundingClientRect().width <= 64))
    assert(await page.locator('.main-content').evaluate(el=>el.scrollWidth <= el.clientWidth + 1))
